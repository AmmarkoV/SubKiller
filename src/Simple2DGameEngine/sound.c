...
{
  if (total_loaded_buffers>=NUM_BUFFERS)
    {
      return;
    }
...
int LoadSoundBuffers()
{
  ...
  if (total_loaded_buffers==0)
    {
      fprintf(stderr,"No Sounds to load!\n");
      return 0;
    }
  ...
  unsigned int i=0;
  for (i=0; i<total_loaded_buffers; i++)
    {
      alutLoadWAVFile((ALbyte *)filenames[i], &format, &data, &size, &freq, &loop);
      theerror=alGetError();
      if(theerror != AL_NO_ERROR)
        {
          fprintf(stderr,"Error (%u) loading Wav sound Buffer %u %s\n",theerror,i,filenames[i]);
        }
  ...
  if(theerror != AL_NO_ERROR)
    {
      fprintf(stderr,"Error Generating Sources (%u)!\n",theerror);
      return 0;
    }
...
void SoundLibrary_LoopSound(unsigned int num)
{
  ...
  if (num<total_loaded_buffers)
    {
      alSourcei (Sources[num], AL_LOOPING,  AL_TRUE          );
      alSourcePlay(Sources[num]);
    }
}
